...
data class GroundScan(val xOffset: Int = 0, val grid: Array<Array<GroundGridElement>> = emptyArray(), val maxX: Int, val maxY: Int, val minY: Int) {
    ...
    fun countWateryElements() = countGridElements(setOf(GroundGridElement.WET_SAND, GroundGridElement.WATER))

    fun countGridElements(elements: Set<GroundGridElement>) = grid.mapIndexed { i, row ->
        if (i >= minY) // Don't count lines above minimum y value
            row.filter { it in elements  }
                    .count()
        else 0
    }.sum()
    ...
    describe("part 1 and 2") {
        ...
        describe("let the water flow") {
            ...
            it("should simulate example") {
                ...
                scan.toString() `should equal` """
                    ...
                    """.trimIndent()
                scan.countWateryElements() `should equal` 57
            }
            it("should simulate case where two flows colide") {
                ...
                scan.toString() `should equal` """
                    ...
                    """.trimIndent()
                scan.countWateryElements() `should equal` 58
            }
        }
        ...
        given("exercise input") {
            ...
            it("should have the water flown correctly") {
                simulateWaterFlow(listOf(springCoord), scan)
                print(scan)
                scan.countWateryElements() `should equal` 31641 // Part 1
                scan.countGridElements(setOf(GroundGridElement.WATER)) `should equal` 26321 // Part 2
            }
        }
    }
})
