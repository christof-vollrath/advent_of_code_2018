...
data class LicenseNode(val metadata: List<Int>, val subNodes: List<LicenseNode> = emptyList()) {
    ...
    fun calculateCheckData(): Int = if (subNodes.isEmpty()) metadata.sum()
    else {
        metadata.map { index ->
...
class Day08Spec : Spek({
    ...
    describe("part 2") {
        given("example") {
            ...
            it("should calculate checkData") {
                val sum = parseLicenseTree(input).calculateCheckData()
                sum `should equal` 66
            }
        }
        given("exercise") {
            val exerciseInput = readResource("day08Input.txt")
            val checkData = parseLicenseTree(exerciseInput).calculateCheckData()
            checkData `should equal` 19724
        }
    }
})
